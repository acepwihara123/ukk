...
if (!empty($search)) {
    ...
    $sql = "SELECT * FROM kit_kamera
            WHERE (nama_kit LIKE '%$search%'
            OR merk LIKE '%$search%'
            OR isi_kit LIKE '%$search%')
            AND status = 'aktif'
            AND stok > 0";
} else {
    $sql = "SELECT * FROM kit_kamera WHERE status = 'aktif' AND stok > 0 ORDER BY id DESC";
}
...
<head>
    ...
    <title>Rental Kit Kamera - Sewa Paket Kamera Profesional</title>
...
<body class="bg-gray-50">
        ...
        <div class="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8">
            <div class="flex justify-between items-center py-3">
                ...
                <div class="hidden md:flex space-x-6">
                    <a href="index.php" class="text-gray-700 hover:text-red-600 font-medium">Home</a>
                    <a href="kamera.php" class="text-gray-700 hover:text-red-600 font-medium">Kamera</a>
                    <a href="kit.php" class="text-red-600 font-medium border-b-2 border-red-600">Kit Kamera</a>
                </div>
                <div class="flex items-center space-x-3">
                    ...
                    <?php else: ?>
                        ...
                        <button onclick="alert('Silakan login terlebih dahulu untuk memesan kit kamera')" class="bg-red-600 hover:bg-red-700 text-white font-medium py-1.5 px-4 rounded-full transition duration-300 text-sm">
    ...
    <div class="hero-bg text-white py-16 md:py-24">
        <div class="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 text-center">
            <h1 class="text-3xl md:text-5xl font-bold mb-4">Sewa Paket <span class="text-red-400">Kit Kamera</span> Profesional</h1>
            <p class="text-lg md:text-xl mb-8 max-w-3xl mx-auto">Dapatkan paket lengkap kamera + lensa + aksesori dengan harga hemat untuk kebutuhan fotografi Anda.</p>
            <form method="GET" class="mb-8 flex justify-center">
                <input type="text" name="search"
                    placeholder="Cari nama kit / merk kamera..."
                    value="<?= $_GET['search'] ?? '' ?>"
            ...
            <?php if (!empty($search)): ?>
                <div class="text-left mb-4">
                    ...
                    <p class="text-gray-300 text-sm">Ditemukan <?= $total ?> kit kamera</p>
                </div>
            <?php endif; ?>
            ...
            <div class="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-4 gap-5">
                    ...
                    <?php while($data = mysqli_fetch_assoc($query)) { ?>
                        <div class="bg-white rounded-xl overflow-hidden shadow-md card-hover">
                            <div class="relative">
                                ...
                                <div class="absolute bottom-2 left-2 bg-purple-500 text-white text-xs px-2 py-0.5 rounded-full">
                                    <i class="fas fa-box-open mr-1"></i> Kit
                                </div>
                            </div>
                            <div class="p-3">
                                <h3 class="text-base font-bold text-gray-900 mb-1 truncate">
                                    <?php echo $data['nama_kit']; ?>
                                </h3>
                                <p class="text-gray-600 text-xs mb-2">
                                    <i class="fas fa-tag mr-1"></i> Merk: <?php echo $data['merk']; ?>
                                </p>
                                <?php if(!empty($data['isi_kit']) && $data['isi_kit'] != '-'): ?>
                                <div class="text-gray-500 text-xs mb-2 line-clamp-2">
                                    <i class="fas fa-list mr-1"></i> 
                                    <?php 
                                    $isi_kit = $data['isi_kit'];
                                    echo strlen($isi_kit) > 50 ? substr($isi_kit, 0, 50) . '...' : $isi_kit;
                                    ?>
                                </div>
                                <?php endif; ?>
                                ...
                                <?php if(isset($_SESSION['user_logged_in'])): ?>
                                    <a href="detail_kit.php?id=<?php echo $data['id']; ?>" 
                                       ...
                                       <i class="fas fa-shopping-cart mr-1"></i> Sewa Kit Ini
                                    </a>
                                <?php else: ?>
                                    <a href="login.php" 
                                       class="w-full bg-red-600 hover:bg-red-700 text-white font-semibold py-1.5 rounded-lg transition duration-300 text-center block text-sm">
                ...
                <?php else: ?>
                    <div class="col-span-4 text-center py-12">
                        ...
                        <p class="text-gray-500 text-lg">Tidak ada kit kamera tersedia</p>
    ...
    <div class="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 py-12">
        <div class="text-center mb-10">
            <p class="text-red-600 font-semibold text-base mb-1">PAKET HEMAT</p>
            <h2 class="text-2xl md:text-3xl font-bold text-gray-900 mb-3">Kit Kamera <span class="text-red-600">Lengkap</span> dengan Harga Hemat</h2>
            <p class="text-gray-600 text-sm max-w-2xl mx-auto">Dapatkan paket kamera + lensa + aksesori dengan harga lebih hemat dari sewa terpisah.</p>
        </div>

        <div class="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-4 gap-5">
            <?php 
            if (empty($search)) {
                $sql_all = "SELECT * FROM kit_kamera WHERE status = 'aktif' AND stok > 0 ORDER BY id DESC LIMIT 8";
            ...
            <?php 
                    ...
                    echo '<div class="col-span-4 text-center py-12">
                            <i class="fas fa-box-open text-5xl text-gray-400 mb-3"></i>
                            <p class="text-gray-500 text-lg">Belum ada kit kamera tersedia</p>
                            <p class="text-gray-400 text-sm mt-1">Silakan cek kembali nanti</p>
                          </div>';
                }
            }
        ...
        <div class="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8">
            <div class="grid grid-cols-1 md:grid-cols-4 gap-6">
                <div>
                    ...
                    <p class="text-gray-400 text-sm mb-4">Menyediakan layanan sewa kit kamera profesional terlengkap.</p>
